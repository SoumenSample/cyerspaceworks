          ...
          <Card>
            ...
            <CardContent>
              ...
              {adminMode === "template" ? (
                ...
                <div className="flex items-center justify-between gap-3">
                  <SectionLabel>Adjustment Rows</SectionLabel>
                ...
                <SectionLabel>Terms & Payment</SectionLabel>
                <div className="grid gap-3 md:grid-cols-2">
                  <Field label="Note 1">
                    <textarea
                      value={form.notes[0]}
          ...
          <Card>
            ...
            <CardContent>
              {loading ? (
                ...
                <p className="text-sm text-cyan-100/70">No bills or invoices have been created yet.</p>
              ) : (
                <div className="space-y-3">
                  {bills.map((bill) => (
                    <div key={bill._id} className={`rounded-lg border p-3 ${selectedId === bill._id ? "border-cyan-400/70 bg-cyan-500/10" : "border-cyan-500/20 bg-cyan-500/5"}`}>
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <button type="button" className="text-left" onClick={() => setSelectedId(bill._id)}>
                          <p className="text-xs uppercase tracking-[0.18em] text-cyan-100/70">{bill.type} {isUploadedRecord(bill) ? "- uploaded" : "- template"}</p>
                          <p className="text-lg font-semibold text-cyan-50">{bill.referenceNumber}</p>
                          <p className="text-sm text-cyan-100/80">{bill.recipientName}</p>
                        </button>
...
function SectionLabel({ children }) {
  return <p className="text-sm font-semibold uppercase tracking-[0.2em] text-cyan-100/80">{children}</p>;
}
