...
const statusStyles = {
  ...
  "in-progress": "bg-cyan-500/15 text-cyan-100",
...
export default function ProjectTimelineBoard({ role, sessionUserId, canEditTasks = false, project, onProjectUpdated, onRefresh, onEditProject }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!project ? (
        <Card>
          <CardContent className="py-16 text-center text-cyan-100/70">
        ...
        <Card className="border-cyan-400/20 bg-black/55">
          <CardHeader className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs uppercase tracking-[0.18em] text-cyan-100/75">
              ...
              {role === "admin" ? (
                <button
                  ...
                  className="rounded-full border border-cyan-400/35 bg-cyan-500/10 px-3 py-1 text-cyan-50 transition-colors hover:bg-cyan-500/20"
                >
                  Edit details
                </button>
              ) : null}
          ...
          <CardContent className="space-y-5">
            ...
            <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
                ...
                <div className="min-w-[1200px] space-y-3">
                  ...
                  >
                    {timeline.days.map((day) => (
                      <div key={day.toISOString()} className="px-2 text-[10px] uppercase tracking-[0.22em] text-cyan-200/55">
                        <span className="block text-[9px] text-cyan-100/30">{day.toLocaleDateString("en", { weekday: "short" }).toUpperCase()}</span>
                        {formatDay(day)}
                      </div>
                    ))}
                  </div>
                      ...
                      <div className="relative p-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1 space-y-1">
                            <h3 className="truncate text-[15px] font-semibold leading-tight text-cyan-50">{project.title}</h3>
                            <p className="text-[11px] leading-snug text-cyan-100/50">
                              {formatLongDate(startDate || new Date())} to {formatLongDate(deadlineDate || new Date())}
                            </p>
                            <div className="flex flex-wrap items-center gap-2 pt-1 text-[10px] uppercase tracking-[0.18em] text-cyan-100/70">
                              ...
                              <span className="rounded-full border border-cyan-500/20 bg-cyan-500/10 px-2.5 py-1">
              ...
              <aside className="space-y-4 rounded-[24px] border border-cyan-500/12 bg-black/25 p-4">
                ...
                {role !== "client" ? (
                  <div className="space-y-3">
                    <div className="text-xs uppercase tracking-[0.2em] text-cyan-100/50">Tasks</div>
                    <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1">
                      {(project.tasks || []).map((task) => (
                        <div key={task._id || task.id || task.title} className="space-y-2 rounded-2xl border border-cyan-500/10 bg-black/25 p-3">
                          <label className="flex items-start gap-3">
                            <input
                              ...
                              className="mt-1 h-4 w-4 rounded border-cyan-400/50 bg-black/40 text-cyan-400"
                            />
                            <span className="min-w-0">
                              <span className="block truncate text-sm font-medium text-cyan-50">{task.title}</span>
                              {task.description ? <span className="block text-[11px] leading-snug text-cyan-100/55">{task.description}</span> : null}
                            </span>
                          </label>
                          ...
                          {!!task.subtasks?.length && (
                            <div className="space-y-2 pl-7">
                              {task.subtasks.map((subtask, subtaskIndex) => (
                                <label key={subtask._id || subtask.id || `${task._id || task.id}-subtask-${subtaskIndex}`} className="flex items-center gap-3 text-xs text-cyan-100/75">
                                  <input
                                    type="checkbox"
                                    ...
                                    className="h-4 w-4 rounded border-cyan-400/50 bg-black/40 text-cyan-400"
                                  />
                ...
                <div className="min-w-[1200px] space-y-3">
                  <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${timeline.days.length}, minmax(${dayWidth}px, ${dayWidth}px))` }}>
                    {timeline.days.map((day) => {
                      ...
                      return (
                        <div key={`done-${day.toISOString()}`} className="min-h-[88px] rounded-xl border border-cyan-500/10 bg-black/25 p-2">
                          <p className="text-[10px] uppercase tracking-[0.18em] text-cyan-100/45">Done {formatDay(day)}</p>
                          {doneItems.length ? (
                            <div className="mt-2 space-y-1.5">
                              {doneItems.map((item, index) => (
                                <div key={`${dayKey}-${item.type}-${index}`} className="rounded-lg border border-emerald-500/25 bg-emerald-500/10 px-2 py-1 text-[11px] text-emerald-100">
                                  {item.type === "subtask" ? `${item.parent}: ${item.title}` : item.title}
                                </div>
                              ))}
                            </div>
                          ) : (
                            <p className="mt-2 text-[11px] text-cyan-100/35">No task done</p>
                          )}
                        </div>
                      );
                    })}
